...
- [x] **M1 — CLI monitoring + explicit manual/auto control (FR-M1).**
...
| M1    | done   | (pending commit) | daemon `config` module (`DaemonConfig{auto_prune}`, TOML at `<store>/daemon.toml`, default `true`, created on first `serve`); `ChunkStore::prune` now takes a `PruneMode{Auto,Manual}` and records it (+`gc`'s own last-run) in a new `status_meta` redb table; `PutManifest` records the uploading client's name (new `snapshot_owner` table, resolved via cert fingerprint, best-effort `None` if unattributed) and, when `auto_prune` is on, runs a grid prune right after the snapshot commits (best-effort, never fails the RPC); `serve` also spawns a 24h safety-net prune timer when `auto_prune` is on. New `ChunkStore::status()` (pure read txn: snapshot totals + per-client breakdown, unique chunks, store bytes, zero-ref chunks, last prune mode/time, last gc time) backs new `busyncr-daemon status [--json]`. Client gains `backup`/`restore --quiet`/`--json-progress` (new `progress` module, TTY-aware `\r` line vs log-safe line-per-interval vs NDJSON, throttled, sourced from the exact FR3 report counters via new `run_backup_with_progress`/`run_restore_with_progress` — the old `run_backup`/`run_restore` are now thin no-op-callback wrappers so no existing call site changed behavior) and `busyncr-client status [--json]` (identity, committed chunk size, a new persisted `status` module's `last-backup.toml` record written after every manual or scheduled backup, last N snapshots when the daemon is reachable). README gained a "Manual vs scheduled operation" subsection. Deviations: (1) FR-M1 M3.2's "`busyncr-daemon status`... safe while `serve` runs" is honored for the case that's actually safe — `ChunkStore::status()` is a plain MVCC read txn, safe against a concurrent `serve` task sharing the same in-process `Arc<ChunkStore>` (proven by `frm1_status_and_prune_mode.rs`) — but redb takes an exclusive OS file lock for a writer's entire open lifetime, so a genuinely separate `busyncr-daemon status` process against a store a running `serve` process already holds open fails with a clean, typed error rather than reading live; documented in the CLI's own doc comment and `--help`, not silently reinterpreted; no cross-process live-status RPC exists in v1. (2) the Windows-only `#[cfg(windows)]` addition to `service.rs` (persisting the last-backup record after each scheduled tick) could not be compiled or run locally (no Windows target, no network to add one) — reviewed by hand against the existing S11 Windows service code it extends; full verification remains blocked on the Windows CI runner, same caveat S11 already carries. Full workspace gate green. |
